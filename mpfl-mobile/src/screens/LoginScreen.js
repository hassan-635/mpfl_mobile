...
const LoginScreen = ({ navigation }) => {
  ...
  const handleLogin = async () => {
    try {
      ...
      const res = await axios.post('https://mpfl-backend.onrender.com/api/v1/auth/login', { email, password });
      if (res.data.token) {
...
const styles = StyleSheet.create({
  ...
  input: { backgroundColor: '#FFFFFF', padding: 18, borderRadius: 15, marginBottom: 15, borderWidth: 1, borderColor: '#E2E8F0' },
